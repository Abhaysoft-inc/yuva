<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            &raquo; Add New Member / नया सदस्य जोड़ें
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('members.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <fieldset class="border border-gray-200 rounded-lg p-4 mb-6">
                            <legend class="text-lg font-semibold text-gray-700 px-2">Select SHG / एसएचजी चुनें</legend>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                <div>
                                    <x-input-label for="shg_id" value="SHG Group / एसएचजी समूह *" />
                                    <select id="shg_id" name="shg_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                        <option value="">-- Select SHG / एसएचजी चुनें --</option>
                                        @foreach($shgs as $shg)
                                            <option value="{{ $shg->id }}" {{ old('shg_id') == $shg->id ? 'selected' : '' }}>
                                                {{ $shg->shg_name }} ({{ $shg->shg_code ?? 'N/A' }})
                                            </option>
                                        @endforeach
                                    </select>
                                    <x-input-error :messages="$errors->get('shg_id')" class="mt-2" />
                                </div>

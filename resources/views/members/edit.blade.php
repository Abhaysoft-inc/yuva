        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('shgs.members.update', [$shg, $member]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <fieldset class="border border-gray-200 rounded-lg p-4 mb-6">
                            ...
                            <div class="mt-4">
                                <x-input-label value="Membership ID / सदस्य आईडी" />
                                <p class="mt-1 text-lg font-bold text-indigo-700">{{ $member->member_id_code }}</p>
                            </div>
                        </fieldset>
                        ...
                        <fieldset class="border border-gray-200 rounded-lg p-4 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                ...
                                <div>
                                    ...
                                    <x-text-input id="husband_father_name" name="husband_father_name" type="text" class="mt-1 block w-full" :value="old('husband_father_name', $member->husband_father_name)" />
                                </div>
                                <div>
                                    <x-input-label for="role" value="Role / भूमिका" />
                                    <select id="role" name="role" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                        <option value="member" {{ old('role', $member->role) == 'member' ? 'selected' : '' }}>Member / सदस्य</option>
                                        <option value="president" {{ old('role', $member->role) == 'president' ? 'selected' : '' }}>President / अध्यक्ष</option>
                                        <option value="secretary" {{ old('role', $member->role) == 'secretary' ? 'selected' : '' }}>Secretary / सचिव</option>
                                        <option value="treasurer" {{ old('role', $member->role) == 'treasurer' ? 'selected' : '' }}>Treasurer / कोषाध्यक्ष</option>
                                    </select>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="mobile" name="mobile" type="text" class="mt-1 block w-full" :value="old('mobile', $member->mobile)" placeholder="10-digit Mobile" />
                                </div>
                                <div>
                                    <x-input-label for="aadhar_number" value="Aadhaar Number / आधार नंबर" />
                                    <x-text-input id="aadhar_number" name="aadhar_number" type="text" class="mt-1 block w-full" :value="old('aadhar_number', $member->aadhar_number)" maxlength="12" />
                                </div>
                                <div>
                                    <x-input-label for="pan_number" value="PAN Card Number / पैन कार्ड नंबर" />
                                    <x-text-input id="pan_number" name="pan_number" type="text" class="mt-1 block w-full" :value="old('pan_number', $member->pan_number)" maxlength="10" />
                                </div>
                                <div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="address" name="address" rows="2" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('address', $member->address) }}</textarea>
                                </div>
                            </div>
                        </fieldset>
                        ...
                        <fieldset class="border border-gray-200 rounded-lg p-4 mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                <div>
                                    ...
                                    <x-text-input id="bank_name" name="bank_name" type="text" class="mt-1 block w-full" :value="old('bank_name', $member->bank_name)" />
                                </div>
                                <div>
                                    <x-input-label for="branch" value="Branch / शाखा" />
                                    <x-text-input id="branch" name="branch" type="text" class="mt-1 block w-full" :value="old('branch', $member->branch)" />
                                </div>
                                <div>
                                    <x-input-label for="account_number" value="Account Number / खाता नंबर" />
                                    <x-text-input id="account_number" name="account_number" type="text" class="mt-1 block w-full" :value="old('account_number', $member->account_number)" />
                                </div>
                                <div>
                                    <x-input-label for="ifsc_code" value="IFSC Code / आईएफएससी कोड" />
                                    <x-text-input id="ifsc_code" name="ifsc_code" type="text" class="mt-1 block w-full" :value="old('ifsc_code', $member->ifsc_code)" />
                                </div>
                            </div>
                        </fieldset>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                <div>
                                    ...
                                    @if($member->passport_photo)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $member->passport_photo) }}" alt="Current Photo" class="w-16 h-16 object-cover rounded border">
                                            <p class="text-xs text-green-600">Upload new to replace</p>
                                        </div>
                                    @endif
                                ...
                                <div>
                                    ...
                                    @if($member->aadhar_card_doc)
                                        <p class="text-sm text-green-600 mb-1">File uploaded. Upload new to replace.</p>
                                    @endif

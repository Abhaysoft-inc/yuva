<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('slides.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition">
                ← Back to Slides
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <img src="{{ asset('storage/' . $slide->image_path) }}" alt="{{ $slide->title }}" class="max-w-full h-auto rounded-lg shadow">
                    </div>
                    @endif
                    ...
                    <form action="{{ route('slides.update', $slide) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                                Change Image (Optional) <span class="text-gray-500 text-xs">(Recommended: 1920x800px)</span>
                            </label>
                            ...
                            <div id="imagePreview" class="mt-4 hidden">
                                <img id="previewImg" src="" alt="Preview" class="max-w-full h-auto rounded-lg shadow">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $slide->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ $slide->is_active ? 'checked' : '' }}
                        ...
                        <div class="flex gap-4">
                            ...
                            <a href="{{ route('slides.index') }}" class="flex-1 text-center px-6 py-3 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition">
                                Cancel
                            </a>
                        </div>
                    </form>
